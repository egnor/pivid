...
class ThreadFrameWindow : public FrameWindow {
  public:
    ...
    virtual Results results() const {
        std::scoped_lock const lock{shared->mutex};
    ...
    void open(
        ...
        auto const filename = media_decoder->file_info().filename;
    ...
    void update_priority(std::unique_lock<std::mutex> const& lock) {
        ...
        for (;;) {
            ...
            if (loaded_eof || tail >= request.end) {  // Fully satisfied
                if (request.freeze) {
                    std::scoped_lock decoder_lock{decoder_mutex};
                    decoder = nullptr;
                }
                if (priority_iter != shared->by_priority.end()) {
                    shared->by_priority.erase(priority_iter);
...
class ThreadFrameLoader : public OldFrameLoader {
  public:
...
std::unique_ptr<OldFrameLoader> make_frame_loader(DisplayDriver* display) {
